...
if __name__ == "__main__":
    ...
    da_opf_n_buses = [key for key in daopf_times.keys()]
    admm_n_buses = [key for key in admm_times.keys()]
    ...
    axs.plot(admm_n_buses[:-1], daopf_list[0:-3], marker='o', label='Fully Centralized')
    axs.plot(admm_n_buses[:-1], admm_sequential_list[:-1], marker='v', label='ADMM (sequential)')
    axs.plot(admm_n_buses[:-1], admm_parallel_list[:-1], marker='s', label='ADMM (parallel)')
